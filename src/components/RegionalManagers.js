...
export default function RegionalManagers() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        setLoading(true);
        const response = await axios.get("https://education-consultancy-backend.onrender.com/api/v1/user"); // update with your actual API
        const users = response.data.data || [];

        // Filter users with role === "admin"
        const filteredAdmins = users.filter((user) => user.Branch === branch && user.RegionalStatus === "Manager");
        ...
        {admins.map((admin, index) => (
          ...
          >
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 p-4">
              <img
                src={`https://education-consultancy-backend.onrender.com/${admin.image}`} // fallback image
