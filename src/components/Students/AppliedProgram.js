...
const AppliedProgram = ({ user_id }) => {
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      ...
      await axios.post("https://education-consultancy-backend.onrender.com/api/v1/studentComment/create", formData);
      setNewComment("");
  ...
  const handleReplySubmit = async (commentId) => {
    ...
    try {
      ...
      await axios.post("https://education-consultancy-backend.onrender.com/api/v1/studentReply/create", formData);
      setReplyContent((prev) => ({ ...prev, [commentId]: "" }));
  ...
  const handleKCCommentSubmit = async () => {
    ...
    try {
      ...
      await axios.post("https://education-consultancy-backend.onrender.com/api/v1/kcComment/create", formData);
      setNewKCComment("");
  ...
  const handleKCReplySubmit = async (commentId) => {
    ...
    try {
      ...
      await axios.post("https://education-consultancy-backend.onrender.com/api/v1/kcReply/create", formData);
      setReplyKCContent((prev) => ({ ...prev, [commentId]: "" }));
      ...
      {comments.length === 0 ? (
        ...
        comments.map((comment) => (
          <div key={comment.id} className="border p-3 rounded-md bg-gray-50">
            ...
            {comment.file && (
              <a
              href={`https://education-consultancy-backend.onrender.com/${comment.file}`}
                target="_blank"
            ...
            <div className="ml-4 space-y-2">
              {comment[repliesKey]?.map((reply) => (
                <div key={reply.id} className="text-sm bg-white p-2 rounded border">
                  ...
                  {reply.file && (
                    <a
                      href={`https://education-consultancy-backend.onrender.com/${reply.file}`}
                      target="_blank"
                      rel="noopener noreferrer"
