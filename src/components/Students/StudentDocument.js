...
//   const handleUpdateAdditionalDoc = async () => {
//     try {
...
const StudentDocument = ({ id }) => {
  ...
  const handleUpdateAdditionalDoc = async (e) => {
    ...
    try {
      const res = await updateAdditionalDocument({ id: editDoc.id, data: formData });
      if (res?.data?.success) {
    ...
    <div className="border rounded-2xl p-4 mb-6 shadow-sm bg-white">
      ...
      <div className="mt-8">
        ...
        <form onSubmit={handleAdditionalDocSubmit} className="flex flex-col md:flex-row gap-4 items-start mb-6">
          <input type="text" name="title" placeholder="Document Title" required className="input border px-3 py-2 rounded w-full md:w-1/2" />
          <input type="file" name="file" accept="application/pdf" required className="input" />
          <p>Limit 5MB</p>
          <button type="submit" className="bg-brandRed text-white px-4 py-2 rounded hover:bg-brandRed-700">Upload</button>
        </form>
          ...
          <div className="space-y-3">
            {additionalDocsData.data.map((doc) => (
              <div key={doc.id} className="flex items-center justify-between p-3 border rounded bg-gray-50 hover:bg-gray-100">
                <a href={`${BASE_URL}${doc.file?.replace(/\\/g, "/")}`} target="_blank" rel="noopener noreferrer" className="text-sm text-brandRed">
                  {doc.title}
                </a>
                <div className="flex gap-3 items-center">
                  ...
                  <FaTrashAlt className="text-red-500 cursor-pointer" onClick={() => handleDeleteAdditionalDoc(doc.id)} />
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div key={idx} className="mb-4">
                  ...
                  <input type="file" accept="application/pdf" onChange={(e) => handleFileChange(e, input.name)} />
                </div>
              ))}
            </div>
            <div className="flex justify-end mt-4">
        ...
        <ModalBody>
          <form onSubmit={handleUpdateAdditionalDoc} className="space-y-4">
            <input
              ...
              required
            />
            <input type="file" accept="application/pdf" onChange={(e) => setEditFile(e.target.files[0])} />
            <div className="flex justify-end gap-3 mt-2">
              <Button onClick={() => setEditModalOpen(false)} className="bg-brandRed">Cancel</Button>
              <Button type="submit" className="bg-brandRed text-white">Update</Button>
            </div>
          </form>
        </ModalBody>
      </Modal>
