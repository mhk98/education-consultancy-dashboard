...
const EnquiriesRequestedPanel = () => {
  ...
  const { data, isLoading, isError, error } = useGetAllEnquiriesQuery({ Status:"archive", page:currentPage, limit:itemsPerPage,});
  ...
  const handleNextSet = () => setStartPage(Math.min(startPage + pagesPerSet, totalPages - pagesPerSet + 1));
   ...
   const { data:data1, isLoading:isLoading1, isError:isError1, error:error1 } = useGetAllEnquiriesQuery({Status:"archive", Branch, page:currentPage, limit:itemsPerPage,});
