...
const EnquiriesRequestedPanel = () => {
 ...
 // Filter out students
 const filtered = allAdminPrograms.filter(program => program.branch === branch && program.status === "archive");
   ...
   const { data:data2, isLoading:isLoading2, isError:isError2, error:error2 } = useGetDataByIdQuery(id);
            ...
            useEffect(() => {
              if (isError2) {
                ...
                const studentPrograms = data2.data
 const filtered = studentPrograms.filter(program => program.status === "archive");
